...
import com.acxiom.librarymgmt.adapters.IssueRequestAdapter;
...
public class IssueRequestsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbarTitle.setText("Issue Requests");
        ...
        adapter = new IssueRequestAdapter(requestList);
    ...
    private void setupHeader() {
        ...
        addHeaderText(header, "Requested Date", 100);
        addHeaderText(header, "Request Fulfilled Date", 100);
    }
    ...
    private void loadData() {
        ...
        firebaseHelper.getDb().collection(Constants.ISSUE_REQUESTS).get()
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    progressBar.setVisibility(View.GONE);
                    requestList.clear();
                    for (QueryDocumentSnapshot doc : queryDocumentSnapshots) {
                        requestList.add(doc.toObject(IssueRequest.class));
                    }
                    adapter.notifyDataSetChanged();
                    tvNoRecords.setVisibility(requestList.isEmpty() ? View.VISIBLE : View.GONE);
                })
